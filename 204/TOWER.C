...
{
	int limit1,limit2,temp,min,dummy,n1;

	if (p ==3) return (n-1);
	limit1 = n_bar(kmax-2,p);
	limit2 = n - n_bar(kmax-1,p) + limit1;
	min = find_m(n-limit1,p-1) + 2*find_m(limit1,p);
	n1 = limit1;
	for (temp = limit1+1;temp<=limit2;temp++) {
		dummy = find_m(n-temp,p-1) + 2*find_m(temp,p);
		if (dummy<=min) {
			min = dummy;
			n1 = temp;
		}
...
int find_m(int n,int p)
{
	int new_n1,kmax,sum,i;

	if (n==0) return 0;
	if (n==1) return 1;
	if (p==3) return (power(n)-1);
	kmax = k_max(n,p);
	new_n1 = find_n1(n,kmax,p);
	return (find_m(n-new_n1,p-1) + 2*find_m(new_n1,p));
}
